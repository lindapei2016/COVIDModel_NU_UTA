...
def evaluate_one_policy_one_sample_path_WA(
    ...
    triggerCase_days = np.sum(np.array(sim_rep.policy.active_indicator_history) == 1)
    triggerHosp_days = np.sum(np.array(sim_rep.policy.active_indicator_history) == 2)
    triggerIcu_days = np.sum(np.array(sim_rep.policy.active_indicator_history) == 3)
    triggerCaseAndHosp_days = np.sum(np.array(sim_rep.policy.active_indicator_history) == 4)

    ICU_difference = np.array(sim_rep.ICU_history).sum(axis=(1, 2))[sim_rep.fixed_kappa_end_date:] - sim_rep.instance.icu
    ICU_violation_patient_days = np.sum(ICU_difference[ICU_difference >= 0])

    ICU_array = np.array(sim_rep.ICU_history).sum(axis=(1, 2))[sim_rep.fixed_kappa_end_date:]
    #print(np.shape(ICU_array))
    IH_array = np.array(sim_rep.IH_history).sum(axis=(1, 2))[sim_rep.fixed_kappa_end_date:]
    IHT_array = ICU_array + IH_array

    ICU_peak = np.round(np.max(ICU_array))
    IHT_peak = np.round(np.max(IHT_array))

    return cost, feasibility, stage1_days, stage2_days, stage3_days, ICU_violation_patient_days, ICU_peak, IHT_peak, \
