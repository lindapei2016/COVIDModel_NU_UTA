...
# Tools_Optimization_Wastewater.py
...
#   and optimization.
# This module is not used to run the SEIR-V model. This module contains
#   functions "on top" of the SEIR-V model.
...
# Use Daily viral load as the indicator in the staged-alert system
###############################################################################
...
from Engine_SimObjects_Wastewater import MultiTierPolicy, CDCTierPolicy
...
def get_sample_paths(
    ...
    #print("debug: {}".format(fixed_kappa_end_date))
    seed = seed_assignment_func(processor_rank)
    rep = SimReplication(city, vaccine_data, None, seed) # no policy?
    ...
    while num_good_reps < goal_num_good_reps:
        ...
        for i in range(len(timepoints)):
            ...
            #print("debug: {}".format(rep.fixed_kappa_end_date))
            rep.simulate_time_period(timepoints[i], fixed_kappa_end_date) # ? bug in the original code?
            #rsq = rep.compute_rsq()
            rsq = rep.compute_rsq_ToIHT()
            print("debug for rsq: {}".format(rsq))
            if rsq < rsq_cutoff:
        ...
        rep = SimReplication(city, vaccine_data, None, None, flag_wastewater_policy=True)
        rep.rng = next_rng
...
def thresholds_generator(stage2_info, stage3_info, stage4_info, stage5_info, flag_tie_allowed=True):
    """
    ...
    # However, ties in thresholds are allowed depends on the flag
    feasible_combos = []
    for combo in candidate_feasible_combos:
        if flag_tie_allowed is True:
...
# why there are only 3 stages? TODO create multitiers version
...
##
# Evaluation Function
def evaluate_one_policy_one_sample_path_wasetwater(policy,
                                                   sim_rep,
                                                   end_time,
                                                   num_stages = 5):
    sim_rep.policy = policy
    #print("debug sim_rep.policy: {}".format(sim_rep.policy))
    start_time = sim_rep.next_t
    sim_rep.fixed_kappa_end_date = start_time
    ...
    feasibility = sim_rep.compute_feasibility_v2(t_start=start_time)
    #print("start_time: {}, debug feasibility: {}".format(start_time, feasibility))

    stage_days = [0 for _ in range(num_stages)]
    for i in range(num_stages):
        stage_days[i] = np.sum(np.array(sim_rep.policy.tier_history) == i)

    # what is sim_rep.instance.dedicated_covid_icu?
    ICU_difference = np.array(sim_rep.ICU_history).sum(axis=(1, 2))[
                     sim_rep.fixed_kappa_end_date:] - sim_rep.instance.dedicated_covid_icu
    ICU_violation_patient_days = np.sum(ICU_difference[ICU_difference >= 0])

    #surge_days = np.sum([i for i in sim_rep.policy.surge_history if i is not None])
    surge_days = None # Sonny's notes: set to None for now
    return cost, feasibility, stage_days, ICU_violation_patient_days, surge_days
...
def evaluate_single_policy_on_sample_path(city: object,
    ...
    for rep in range(num_reps):
        ...
        base_json_filename = str(base_path_filename) + str(rep + 1) + "_" + str(
            kappa_t_end) + "_"
        base_rep = SimReplication(city, vaccines, None, 1, flag_wastewater_policy=True)
