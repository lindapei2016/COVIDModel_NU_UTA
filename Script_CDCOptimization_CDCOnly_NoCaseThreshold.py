...
# Script_CDCOptimization_FinerGrid did not include CDC implemented policy
#   so simulating that
# Splitting it up amongst 30 processors

# Can also repurpose this to quickly simulate a small set of solutions
...
# No thresholds generator function because this is just 1 CDC policy
# (10, 20) & (.1, .2)
        ...
        for policy_id in policy_ids_to_evaluate:
            ...
            for rep in reps:
                ...
                if rep_counter == 10 or rep_counter % 50 == 0 or rep_counter == num_reps_evaluated_per_policy:
                    np.savetxt("rank" + str(rank) + "_peak" + str(peak) + "_policyCDCimplementation" + "_stage2_days.csv",
                               np.array(stage2_days_per_rep), delimiter=",")
                    np.savetxt("rank" + str(rank) + "_peak" + str(peak) + "_policyCDCimplementation" + "_stage3_days.csv",
                               np.array(stage3_days_per_rep), delimiter=",")
                    np.savetxt("rank" + str(rank) + "_peak" + str(peak) + "_policyCDCimplementation" + "_ICU_violation_patient_days.csv",
